...
class Node:
    ...
    def __str__(self):
        return "Level: %s Profit: %s Bound: %s Weight: %s Contains:%s" % (self.level, self.profit, self.bound, self.weight, self.contains)
...
def KnapSackBranchNBound(weight, items, total_items):
    ...
    while (len(Q) != 0):
        ...
        # print(u.contains, items[v.level].index, v.contains)
